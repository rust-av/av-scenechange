...
use super::*;
use crate::data::pixel_from_u16;

fn create_plane<T: Pixel>(width: usize, height: usize) -> Plane<T> {
    let bit_depth = if size_of::<T>() == 1 { 8 } else { 10 };
    FrameBuilder::new(width, height, ChromaSubsampling::Monochrome, bit_depth)
        .build::<T>()
        .unwrap()
        .y_plane
}
...
fn fill_plane<T: Pixel>(plane: &mut Plane<T>, value: i32) {
    ...
    for row in 0..height {
        for col in 0..width {
            data[origin + row * stride + col] = pixel_from_u16(value as u16);
        }
    }
}
...
fn fill_plane_gradient<T: Pixel>(plane: &mut Plane<T>) {
    ...
    for row in 0..height {
        for col in 0..width {
            ...
            data[origin + row * stride + col] = pixel_from_u16(val as u16);
        }
    }
}
