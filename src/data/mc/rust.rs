...
unsafe fn run_filter<T: Pixel>(src: *const T, stride: usize, filter: [i32; 8]) -> i32 {
    filter
        .iter()
        .enumerate()
        .map(|(i, f)| {
...
/// SAFETY: caller must validate that `stride * filter.len() <= src.len()`
unsafe fn run_filter_i16(src: *const i16, stride: usize, filter: [i32; 8]) -> i32 {
    filter
        .iter()
        .enumerate()
        .map(|(i, f)| {
